...
def send_can_frames(angles_rad):
    """Send CAN frames for all 6 joints."""
    try:
        with can.interface.Bus(interface=CAN_INTERFACE, channel=CAN_CHANNEL) as bus:
            for axis_id in range(1, 7):
                idx         = axis_id - 1
                gear_ratio  = GEAR_RATIOS[idx]
                hex_msg     = angle_to_can_message(axis_id, DEFAULT_SPEED, angles_rad[idx], gear_ratio)
                ...
                bus.send(msg)
                rospy.loginfo(f"CAN sent J{axis_id}: arb=0x{arb_id:02X} data={[hex(b) for b in payload]}")
